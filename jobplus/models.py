...
class Company(Base):
    ...
    login_url = db.Column(db.String(64), nullable=False)
...
class Job(Base):
    ...
    requirement = db.Column(db.String(32))
    status = db.Column(db.Boolean, default=True)
    company = db.relationship('Company', uselist=False, backref=db.backref('job', uselist=False))
    company_id = db.Column(db.Integer, db.ForeignKey('company.id', ondelete='SET NULL'))
    applicant = db.relationship('User', secondary, backref='applied_jobs')
